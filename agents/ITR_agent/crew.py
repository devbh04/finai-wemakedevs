...
def _build_task_context(processed_documents, ca_report_data, client_type):
    ...
    if ca_report_data:
        ...
        context_parts.append(f"- Previous client type: {ca_insights.get('client_type', 'Unknown')}")
        context_parts.append(f"- Financial summary: {ca_insights.get('financial_summary', {})}")
        context_parts.append(f"- Tax details: {ca_insights.get('tax_details', {})}")
        context_parts.append(f"- Deductions claimed: {ca_insights.get('deductions_claimed', [])}")
        context_parts.append(f"- Investment details: {ca_insights.get('investment_details', [])}")
        
        # Add raw content preview
        raw_content = ca_report_data.get("raw_content", "")
        if raw_content:
            context_parts.append(f"CA REPORT CONTENT PREVIEW: {raw_content[:1000]}...")
    else:
        context_parts.append("CA REPORT: No previous CA analysis available")
