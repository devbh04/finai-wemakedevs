...
async def analyze_itr_documents(
    ...
    """
    ITR document analysis endpoint with CA report integration
    """
    saved_files = []
    for file in files:
    ...
    try:
        ...
        # Fetch the latest CA report for this client type
        ca_fetcher = CAReportFetcher()
        ca_report_path = ca_fetcher.get_latest_ca_report(client_category)
        
        # Process documents with intelligent CA report matching
        doc_processor = ITRDocumentProcessor(saved_files, ca_report_path, client_category)
        processed_data = doc_processor.process_documents()
        
        # Log matching analysis
        if processed_data.get("matching_analysis"):
            print(f"CA Report Matching Analysis: {processed_data['matching_analysis']}")
            print(f"Using CA Report: {processed_data['ca_report_used']}")
        
        # Create crew with processed documents and CA report data
        crew, task_name = create_crew(
            client_category, 
            processed_data["user_documents"], 
            processed_data["ca_report_data"]
        )
...
- CA Report Available: {'Yes' if ca_report_path else 'No'}
- Matching Analysis: {processed_data.get('matching_analysis', {}).get('recommendation', 'N/A')}
...
"""
        ...
        markdown_content += f"**CA Report Used:** {ca_report_path or 'None'}\n\n"
        ...
        return JSONResponse(content={
            ...
            "ca_report_used": processed_data.get("ca_report_used", False),
            "ca_report_path": processed_data.get("ca_report_path", None),
            "files_processed": len(saved_files),
            "client_category": client_category,
            "matching_analysis": processed_data.get("matching_analysis", {}),
